<?php
defined('BASEPATH') or exit('No direct script access allowed');

class Serverside_m extends CI_Model
{
    var $column_order = array(null, 'nama', 'foto'); //set column field database for datatable orderable
    var $column_search = array('nama'); //set column field database for datatable searchable
    var $order = array('service_id' => 'desc'); // default order 

    private function _get_datatables_query()
    {
        $this->db->select('*');
        $this->db->from('our_service');
        $i = 0;
        foreach ($this->column_search as $blog) { // loop column 
            if (@$_POST['search']['value']) { // if datatable send POST for search
                if ($i === 0) { // name loop
                    $this->db->group_start(); // open bracket. query Where with OR clause better with bracket. because maybe can combine with other WHERE with AND.
                    $this->db->like($blog, $_POST['search']['value']);
                } else {
                    $this->db->or_like($blog, $_POST['search']['value']);
                }
                if (count($this->column_search) - 1 == $i) //last loop
                    $this->db->group_end(); //close bracket
            }
            $i++;
        }

        if (isset($_POST['order'])) { // here order processing
            $this->db->order_by($this->column_order[$_POST['order']['0']['column']], $_POST['order']['0']['dir']);
        } else if (isset($this->order)) {
            $order = $this->order;
            $this->db->order_by(key($order), $order[key($order)]);
        }
    }
    function get_datatables()
    {
        $this->_get_datatables_query();
        if (@$_POST['length'] != -1)
            $this->db->limit(@$_POST['length'], @$_POST['start']);
        $query = $this->db->get();
        return $query->result();
    }
    function count_filtered()
    {
        $this->_get_datatables_query();
        $query = $this->db->get();
        return $query->num_rows();
    }
    function count_all()
    {
        $this->db->from('our_service');
        return $this->db->count_all_results();
    }






    // User
    var $column_order_user = array(null, 'nama', 'foto', 'email', 'telpon', 'status'); //set column field database for datatable orderable
    var $column_search_user = array('nama'); //set column field database for datatable searchable
    var $order_user = array('user_id' => 'desc'); // default order 

    private function _get_user_query()
    {
        $this->db->select('*');
        $this->db->from('user');
        $i = 0;
        foreach ($this->column_search_user as $user) { // loop column 
            if (@$_POST['search']['value']) { // if datatable send POST for search
                if ($i === 0) { // name loop
                    $this->db->group_start(); // open bracket. query Where with OR clause better with bracket. because maybe can combine with other WHERE with AND.
                    $this->db->like($user, $_POST['search']['value']);
                } else {
                    $this->db->or_like($user, $_POST['search']['value']);
                }
                if (count($this->column_search_user) - 1 == $i) //last loop
                    $this->db->group_end(); //close bracket
            }
            $i++;
        }

        if (isset($_POST['order'])) { // here order processing
            $this->db->order_by($this->column_order_user[$_POST['order']['0']['column']], $_POST['order']['0']['dir']);
        } else if (isset($this->order)) {
            $order_user = $this->order;
            $this->db->order_by(key($order_user), $order_user[key($order_user)]);
        }
    }
    function get_datatables_user()
    {
        $this->_get_user_query();
        if (@$_POST['length'] != -1)
            $this->db->limit(@$_POST['length'], @$_POST['start']);
        $query = $this->db->get();
        return $query->result();
    }
    function count_filtered_user()
    {
        $this->_get_user_query();
        $query = $this->db->get();
        return $query->num_rows();
    }
    function count_all_user()
    {
        $this->db->from('user');
        return $this->db->count_all_results();
    }
}
